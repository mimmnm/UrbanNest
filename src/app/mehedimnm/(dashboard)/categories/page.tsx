    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Organize your products ({categories.length} categories)</p>
        </div>
      ...
      {categories.length === 0 ? (
        <div className="text-center py-16">
          <ImageIcon size={48} className="mx-auto text-stone-200 mb-3" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((category, i) => {
            ...
            return (
              <motion.div key={cid} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} whileHover={{ y: -3 }} className="bg-white rounded-2xl border border-stone-100 overflow-hidden transition-shadow hover:shadow-lg">
                <div className="relative h-40 bg-stone-100">
                  ...
                  <div className="absolute bottom-3 left-4">
                    <h3 className="text-white font-medium drop-shadow-sm">{category.name}</h3>
                  </div>
                </div>
                <div className="p-4">
                  {category.description && <p className="text-xs text-stone-500 mb-2 line-clamp-2">{category.description}</p>}
      ...
      <AnimatePresence>
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white rounded-2xl shadow-xl w-full max-w-md">
              <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100">
                <h3 className="text-lg font-medium text-stone-900">{editingId ? "Edit Category" : "Add Category"}</h3>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  {image ? (
                    <div className="relative w-full h-32 rounded-xl overflow-hidden bg-stone-100 mb-2 group">
                    ...
                    <button onClick={() => fileInputRef.current?.click()} className="w-full h-32 rounded-xl border-2 border-dashed border-stone-200 flex flex-col items-center justify-center text-stone-400 hover:border-stone-400 hover:text-stone-600 transition-colors mb-2">
                ...
                <div>
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-300" placeholder="e.g., Skincare" />
                </div>

                <div>
                  <label className="block text-xs text-stone-500 mb-1.5">Description</label>
                  <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-300 resize-none" placeholder="Category description" />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => { setShowModal(false); setSaveError(""); }} className="flex-1 py-3 border border-stone-200 rounded-xl text-sm text-stone-600 hover:bg-stone-50 transition-colors">Cancel</button>
                  <button onClick={handleSave} disabled={saving || !name} className="flex-1 py-3 bg-stone-900 text-white rounded-xl text-sm font-medium hover:bg-stone-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
